...
class RoomStep(CartMixin, TemplateFlowStep):
        ...
        if self.cart_session['room_mode'] == 'join':
            ...
            if self.create_form.is_valid():
                ...
                if self.cart_session.get('room_create'):
                    try:
                        room = Room.objects.get(event=self.event, pk=self.cart_session['room_create'])
                    except Room.DoesNotExist:
                        pass
    ...
    def create_form(self):
        initial = {}
        current = None
        if self.cart_session.get('room_mode') == 'create' and 'room_create' in self.cart_session:
            try:
                current = Room.objects.get(event=self.event, pk=self.cart_session['room_create'])
            except Room.DoesNotExist:
                pass
            else:
        ...
        return RoomCreateForm(
            ...
            initial=initial,
            current=current,
            data=self.request.POST if self.request.method == "POST" and self.request.POST.get("room_mode") == "create" else None
        )

    @cached_property
    def join_form(self):
        initial = {}
        if self.cart_session.get('room_mode') == 'join' and 'room_join' in self.cart_session:
            try:
                room = Room.objects.get(event=self.event, pk=self.cart_session['room_join'])
            except Room.DoesNotExist:
                pass
            else:
        ...
        return RoomJoinForm(
            ...
            prefix='join',
            initial=initial,
            data=self.request.POST if self.request.method == "POST" and self.request.POST.get("room_mode") == "join" else None
        )
    ...
    def is_completed(self, request, warn=False):
        if request.event.has_subevents and cart_session(request).get('room_mode') == 'join' and 'room_join' in cart_session(request):
            try:
                room = Room.objects.get(event=self.event, pk=cart_session(request)['room_join'])
                room_subevents = set(c['order__all_positions__subevent'] for c in room.orderrooms.filter(order__all_positions__canceled=False).values('order__all_positions__subevent').distinct())
                ...
                if room_subevents:
                    ...
                    if any(c not in room_subevents for c in cart_subevents):
                        if warn:
                            messages.warning(request, _('You requested to join a room that participates in "{subevent_room}", while you chose to participate in "{subevent_cart}". Please choose a different room.').format(
                                subevent_room=SubEvent.objects.get(pk=list(room_subevents)[0]).name,
