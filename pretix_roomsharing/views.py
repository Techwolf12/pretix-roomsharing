...
from pretix.base.models import Order, SubEvent, OrderPosition, OrderRefund, Event, Question
...
class RoomsharingSettingsForm(SettingsForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        questionChoices = (
            (str(i["identifier"]), i["question"]) for i in Question.objects.values("question", "identifier").all()
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if mode == "leave":
            try:
                ...
                c.delete()
                self.order.log_action("pretix_roomsharing.order.left", data={
                    'room': c.pk
                })
                messages.success(request, _('Okay, you left your room successfully. How do you want to continue?'))
                return redirect(eventreverse(self.request.event, 'plugins:pretix_roomsharing:event.order.room.modify',
                                             kwargs={
            ...
            if self.change_form.is_valid():
                try:
                    ...
                    if c.is_admin:
                        ...
                        c.room.save()
                    self.order.log_action("pretix_roomsharing.order.changed", data={
                        'room': c.pk
                    })
            ...
            if self.join_form.is_valid():
                ...
                self.order.log_action("pretix_roomsharing.order.joined", data={
                    'room': room.pk
                })
            ...
            if self.create_form.is_valid():
                ...
                self.order.log_action("pretix_roomsharing.order.created", data={
                    'room': room.pk
                })
                messages.success(request, _('Great, we saved your changes!'))
                return redirect(self.get_order_url())
...
class RoomForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if Room.objects.filter(event=self.event, name=name).exclude(pk=self.instance.pk).exists():
            raise forms.ValidationError(
                _('Duplicate room name'),
                code='duplicate_name'
            )
        return name
...
class RoomDelete(EventPermissionRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        o.log_action("pretix_roomsharing.room.deleted", data={
            "name": o.name
        }, user=request.user)
        for oc in self.object.orderrooms.select_related('order'):
            oc.order.log_action("pretix_roomsharing.order.deleted", data={
                'room': o.pk
            }, user=request.user)
            oc.delete()
        o.delete()
...
class StatsView(StatsMixin, EventPermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for d in self.get_ticket_stats(self.request.event):
            ...
            if d.get('qs_cliq'):
                qsc = list(d['qs'].order_by().values('subevent', 'has_room').annotate(c=Count('*'), cc=Count('order__orderroom__room', distinct=True)))
                c1 = qs_by_room(qsc)
...
class MetricsView(StatsMixin, View):
    ...
    def get(self, request, organizer, event):
        ...
        for d in self.get_ticket_stats(event):
            if d.get('qs_cliq'):
                qs = d['qs'].order_by().values('subevent', 'item', 'has_room').annotate(c=Count('*'), cc=Count('order__orderroom__room', distinct=True))
                for r in qs:
                    m[d['id']]['{item="%s",subevent="%s",hasroom="%s"}' % (r['item'], r['subevent'], r['has_room'])] = r['c']
                    if r['cc']:
                        m[d['id'] + '_unique_rooms']['{item="%s",subevent="%s"}' % (r['item'], r['subevent'])] = r['cc']
            else:
